...
if(isset($_POST['department'])){
	...
	if($id>0){
		...
		/*//$count=array(array("0","0"));
		$count=array();
		array_push($count,mysqli_query($con,"select * from department"));
		$count=mysqli_fetch_assoc($count);
		$i=count($count);
		print_r($count)*/
		$sql="insert into department(id,department) values('$departmentid', '$department')";
	}
                        ...
                        <div class="card-body card-block">
                           <form method="post">
							   <div class="form-group">
								<label for="department" class=" form-control-label">Department ID</label>
								<input type="text" value="<?php echo $department?>" name="departmentid" placeholder="Enter your department ID" class="form-control" required>
